...
#define reciproce_sqrt3 0.57735026918962576450914878050196
	...
	float3 smallTrianglesColor(float3 pos)
	{
		...
		/*
		float2 c = (pos.xyz) / _TrianglesScale;
		c = ((c + float2(c.y, 0.0)*cos(a)) / zoom) + float2(floor((c.x - c.y*cos(a)) / zoom*4.0) / 4.0, 0.0);//Add rotations*/
	...
	float3 largeTrianglesColor(float3 pos)
	{
		...
		/*
		float2 c = (pos.xyz) / _TrianglesScale;//scaled coordinates
		c = ((c + float2(c.y, 0.0)*cos(a)) / zoom) +float2(floor((c.x - c.y*cos(a)) / zoom*4.0) / 4.0, 0.0);//Add rotations
		...
		/*
		float3 c = (pos.xyz) * float3(sin(a), 1.0, sin(a)) / _TrianglesScale;//scaled coordinates
		float3 d = ((c + float3(c.y, 0.0, c.y)*cos(a)) / zoom);
		d += float3(floor((c.x - c.y*cos(a)) / zoom*4.0) / 4.0, 0.0, floor((c.z - c.y*cos(a)) / zoom*4.0) / 4.0);//Add rotations
		float l = min(
			min(
				min(
			...
			min(
				min(
					(1.0 - (2.0 * abs(frac((c.z - c.y)*4.0) - 0.5))),
					(1.0 - (2.0 * abs(frac(c.y * 4.0) - 0.5)))
				),
				(1.0 - (2.0 * abs(frac(c.z * 4.0) - 0.5)))
			)
		);*/
	...
	{
		float d = length(pos.xyz - midPos.xyz);
		d /= _RangeScale;
